...
La ricerca nell'ambito dei Sistemi di Trasporto Intelligenti (ITS) e della navigazione assistita ha prodotto nel tempo algoritmi sempre più sofisticati per la ricerca del percorso minimo (es. algoritmi di Dijkstra e A* modificati per reti stradali dinamiche). La letteratura accademica recente, consultabile tramite Google Scholar, evidenzia un interesse crescente non solo verso il "routing" puro, ma verso l'Interazione Uomo-Macchina (HCI) nel contesto della guida: come presentare le informazioni spaziali in modo efficace, riducendo i tempi di reazione del guidatore.
...
2. **Context Awareness misurata:** L'utente viene informato sulle strade laterali solo in condizioni di sicurezza (es. veicolo fermo), ottimizzando le richieste API.
...
1.  **Lateral Road Detection (Rilevamento Contesto Laterale) con Cooldown Logico:**
    A differenza dei navigatori tradizionali che mostrano ciecamente una mappa 2D/3D, la nostra applicazione percepisce attivamente l'ambiente circostante quando l'utente si ferma. L'innovazione sta nell'aver implementato un sistema di *cooldown* e di verifica di stato. L'app non effettua costantemente costose chiamate API per cercare strade laterali: la logica si attiva solo se la velocità scende a zero, se l'utente non si trova in prossimità di un punto di svolta critico (dove le indicazioni di rotta hanno priorità assoluta), e solo dopo un lasso di tempo stabilito per evitare lo spam di avvisi in caso di traffico congestionato (stop-and-go).
...
*   **PlantUML:** Per definire la struttura del software in modo formale (pur non allegando i documenti in questa sede, essi hanno guidato lo sviluppo logico) sono stati progettati diagrammi delle Classi (per i servizi di dominio), diagrammi di Sequenza (per mappare il flusso temporale dal tap della ricerca al render del percorso) e diagrammi degli Stati (per modellare i cambi di stato della UI e del monitor di navigazione).
...
*   `NavigationMonitor`: È il motore dell'applicazione. Si sottoscrive allo stream del GPS. Ad ogni nuovo segnale (es. ogni secondo):
    ...
    *   Rototrasla la telecamera per farle seguire l'orientamento del veicolo (bearing).
...
*   *[INSERIRE QUI SCREENSHOT 5: Overlay di Lateral Road Detection]*
    *   **Descrizione:** Simulando l'arresto del veicolo a un incrocio o in colonna, lo screenshot mostra l'apparizione di un overlay informativo discreto (es. colore arancione o indicazione testuale) che segnala all'utente l'identificazione di strade limitrofe, verificando il corretto funzionamento delle logiche di cooldown e delle API di snap-to-road.
...
3.  Aggiungere il supporto vocale (Text-To-Speech) per la lettura automatica delle istruzioni di svolta, elevando ulteriormente il livello di sicurezza alla guida eliminando la necessità di guardare lo schermo.
